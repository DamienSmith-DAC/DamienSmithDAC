...
#######################################################################################
# Copy MicroStrategy full backup files from dp-mysql-meta-02 to backup-master01.
# This is a data pull.
# This script is called by root cron job scheduled to run
# every day 2 hours after mstr  full backup cronjob on  dp-mysql-meta-02
########################################################################################
# Version: 1.0
# Developer: Indu N
########################################################################################
...
LOCAL_PARENT_FULL_DIR="/mstr/full"
TODAYS_FULL_DIR="${LOCAL_PARENT_FULL_DIR}/$(date "+%Y")/$(date "+%m")"
LOG_PATH="/mstr/log/FULL_COPY_$(date +%a%Y%m%d).log"
...
ssh -i /root/dace2_data_product_root_private_key root@${REMOTE_HOST}  "test -e ${REMOTE_FULL_DIR}"
